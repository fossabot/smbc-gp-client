...
class SmbcGpClient():
    ...
    def create_transaction(self, shopId, shopPass, orderId, jobCd, amount, tax, 
        ...
        Args:
            - shopId: shopId is issued by smbc-gp, begining with tshop*.
            - shopPass: shopPass is issued by smbc-gp, should be 8 char.
            - orderId: orderId is to identified a transaction.
            - jobCd: processing category. available value are CHECK(validity check)/CAPTURE(immediate sales)/AUTH(temparary sales)/SAUTH(simple authorization)
            - itemCode: product code, default '0000990'
            - amount: transaction amount
            - tax: transaction tax
            - tdFlag: whether to use 3DS
            - tdTenantName: show the shop name with base64 encoded
            - tds2Type: determine what to do next if 3DS2.0 not support
            - tdRequired: wheter to use 3DS when execute transaction
